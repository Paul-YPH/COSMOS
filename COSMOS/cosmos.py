...
#         sc.pp.pca(adata2)
    ...
    def train(self, embedding_save_filepath="./embedding.tsv", weights_save_filepath="./weights.tsv", spatial_regularization_strength=0.1, z_dim=50, lr=1e-3, wnn_iter = 100, epochs=1000, max_patience=50, min_stop=100, random_seed=42, gpu=0, regularization_acceleration=True, edge_subset_sz=1000000):
        ...
        :param wnn_iter: the iteration number before performing WNN
        :type wnn_iter: int, optional, default: 100
        ...
        for epoch in range(1,epochs):
            ...
            optimizer.zero_grad()
            if epoch%wnn_iter == 0 and epoch <= wnn_iter:
            ...
            if epoch > wnn_iter:
                if train_loss > min_loss:
                    ...
                    best_params = model.state_dict()
            if epoch % 10 == 1:
                print(f"Epoch {epoch}/{epochs}, Loss: {str(train_loss)}")
...
class GraphEncoderWNNit(nn.Module):
    ...
    def forward(self, x1, x2, edge_index, adata,w,w1,w2):
        ...
        if w==1:
            ...
            w1 = torch.reshape(torch.from_numpy(ww[:,0]),(-1,1))
            w2 = torch.reshape(torch.from_numpy(ww[:,1]),(-1,1))
